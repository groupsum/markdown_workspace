...
The stable cross-package class contract is defined in `@markdown-workspace/theme-contract` and re-exported by `@markdown-workspace/ui-tokens`.
...
These helper classes are exported by `@markdown-workspace/ui-tokens` so renderer packages and applications can share them without depending on `apps/client`.

## Stability guidance

- **stable**: documented in `@markdown-workspace/theme-contract`; safe for package-to-package interoperability
...
- normative contract: `@markdown-workspace/theme-contract`
- implemented CSS and helper classes: `@markdown-workspace/ui-tokens`
