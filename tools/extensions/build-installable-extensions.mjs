...
async function collectPortableFiles(rootDir, currentDir = rootDir) {
  const entries = await fs.promises.readdir(currentDir, { withFileTypes: true });
  const files = [];
  for (const entry of entries) {
    const absolutePath = path.join(currentDir, entry.name);
    if (entry.isDirectory()) {
      files.push(...await collectPortableFiles(rootDir, absolutePath));
      continue;
    }
    files.push({
      path: path.relative(rootDir, absolutePath).split(path.sep).join('/'),
      content: await fs.promises.readFile(absolutePath, 'utf8'),
      mimeType: entry.name.endsWith('.css')
        ? 'text/css'
...
export async function buildInstallableExtensions() {
  ...
  for (const workspacePackage of extensionPackages) {
    ...
    await writeJson(path.join(versionRoot, 'portable-package.json'), {
      kind: 'mdwrk-extension-package',
      version: 1,
      catalogId: manifest.publisher ?? manifest.id,
      entryId: createCatalogEntryId(manifest.id, manifest.version),
      files: await collectPortableFiles(versionRoot),
    });
