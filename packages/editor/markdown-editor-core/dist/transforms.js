...
export function getSelectedLineRange(value, selection) {
...
function getCurrentLineRange(value, offset) {
...
function applyBulletPrefix(line) {
...
export function applyBulletListSelection(value, selection) {
    const normalized = normalizeSelection(selection, value.length);
...
export function applyTaskListSelection(value, selection) {
    const normalized = normalizeSelection(selection, value.length);
    const { lineStart, lineEnd, lines } = getSelectedLineRange(value, normalized);
...
export function getListContinuationPrefix(line) {
...
export function isEmptyListItemLine(line) {
    return (/^\s*[-*+](?:\s+)?$/.test(line) ||
        /^\s*[-*+]\s+\[[ xX]\]\s*$/.test(line) ||
        /^\s*\d+[.)](?:\s+)?$/.test(line) ||
        /^\s*\d+[.)]\s+\[[ xX]\]\s*$/.test(line));
}
export function insertListContinuation(value, selection) {
    const normalized = normalizeSelection(selection, value.length);
...
function isSelectionWrappedBy(value, selection, prefix, suffix = prefix) {
    const normalized = normalizeSelection(selection, value.length);
    if (normalized.start === normalized.end) {
        return false;
    }
...
function isCursorWithinWrapped(value, offset, prefix, suffix = prefix) {
    const safeOffset = Math.max(0, Math.min(value.length, offset));
...
export function computeSelectionFormatState(value, selection) {
    const normalized = normalizeSelection(selection, value.length);
