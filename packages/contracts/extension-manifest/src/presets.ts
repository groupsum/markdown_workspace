...
export type CapabilityPresetId =
  | "workspace.module.standard"
  | "workspace.module.reader"
...
export type CapabilityPresetRequiredContributionKind =
  | "views"
  | "workspaceModules"
  | "actionRail"
  | "settingsSections";
...
export type CapabilityPresetGapKind =
  | "unknown-preset"
  | "missing-capability"
  | "missing-profile"
...
export const CAPABILITY_PRESETS: readonly CapabilityPresetDescriptor[] = [
  {
...
function appendUnique<T extends string>(target: T[], values: readonly T[]): void {
...
function getWorkspaceModuleProfileIds(manifest: ExtensionManifest): readonly WorkspaceModuleCapabilityProfile[] {
  const profileIds: WorkspaceModuleCapabilityProfile[] = [];
...
export function getCapabilityPresetDescriptor(id: CapabilityPresetId): CapabilityPresetDescriptor {
...
export function composeCapabilityPresetSet(ids: CapabilityPresetSet): CapabilityPresetComposition {
  const normalizedIds: CapabilityPresetId[] = [];
  const profileIds: WorkspaceModuleCapabilityProfile[] = [];
  const recommendedCapabilities: ExtensionCapability[] = [];
  const requiredContributionKinds: CapabilityPresetRequiredContributionKind[] = [];
  const notes: string[] = [];
...
export function recommendCapabilityPresets(manifest: ExtensionManifest): CapabilityPresetSet {
  if (!manifest.contributions.workspaceModules?.length) {
    return [];
  }

  const profileIds = new Set(getWorkspaceModuleProfileIds(manifest));
  const recommendations: CapabilityPresetId[] = [];
  const add = (id: CapabilityPresetId) => {
...
export function evaluateCapabilityPresetSet(
  manifest: ExtensionManifest,
  ids: readonly string[],
): CapabilityPresetEvaluation {
  const knownIds: CapabilityPresetId[] = [];
  const gaps: CapabilityPresetAdvisoryGap[] = [];

  for (const id of ids) {
    if (!CAPABILITY_PRESET_BY_ID.has(id as CapabilityPresetId)) {
      gaps.push({
        kind: "unknown-preset",
        id,
        presetId: id,
    ...
    knownIds.push(id as CapabilityPresetId);
  }
