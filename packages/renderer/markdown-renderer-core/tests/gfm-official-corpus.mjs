...
const fixturePath = path.join(testDir, 'fixtures', 'official', 'gfm-0.29-gfm-spec.txt');
...
const summary = {
  spec: 'GFM 0.29-gfm',
  engine,
  fixturePath: 'packages/renderer/markdown-renderer-core/tests/fixtures/official/gfm-0.29-gfm-spec.txt',
  total: tests.length,
  passed,
...
if (process.argv.includes('--json')) {
  ...
  assert.equal(summary.failed, 0, `gfm official corpus failures: ${summary.failed}`);
  console.log(`gfm official corpus: ${summary.passed}/${summary.total} via ${engine}`);
}
