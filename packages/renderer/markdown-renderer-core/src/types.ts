...
export interface MarkdownBlockNode {
  ...
  readonly header?: readonly Readonly<{ text: string; align?: 'left' | 'center' | 'right' }>[];
...
export interface MarkdownAstRoot {
  readonly type: 'root';
  readonly children: readonly MarkdownBlockNode[];
  readonly references: Readonly<Record<string, { readonly url: string; readonly title?: string }>>;
...
export type MarkdownOptionalProfileId =
  | 'front-matter'
  | 'footnotes'
  | 'definition-lists'
  | 'math'
