...
if ! rg -n "^## Run log" "$NOTES_FILE" >/dev/null; then
  printf '\n## Run log\n' >> "$NOTES_FILE"
fi

python - <<'PY' "$NOTES_FILE" "$DATE_STAMP" "$TIME_STAMP" "$BURST_NOTE"
...
marker = "## Run log\n"
entry = f"- {date_stamp} {time_stamp}: {burst_note}.\n"
...
echo "Added run log entry to $NOTES_FILE"
