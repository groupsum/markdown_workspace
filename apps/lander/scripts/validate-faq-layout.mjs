...
assert.doesNotMatch(blogView, /AnswerBlocks|extractTerminalAnswerBlocks|article-guide/, 'Product updates SPA fallback must not render answer blocks.');
...
assert.match(blogView, /stripLegacyAeoSections/, 'Product updates SPA fallback must strip legacy Quick Reference and Article Guide sections.');
...
assert.match(blogView, /blog-post-subtitle/, 'Product updates SPA fallback must render subtitles when present.');
