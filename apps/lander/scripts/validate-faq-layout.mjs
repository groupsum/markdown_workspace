...
assert.doesNotMatch(blogView, /AnswerBlocks|extractTerminalAnswerBlocks|article-guide/, 'Blog SPA fallback must not render answer blocks.');
...
assert.match(blogView, /stripLegacyAeoSections/, 'Blog SPA fallback must strip legacy Quick Reference and Article Guide sections.');
...
assert.match(blogView, /blog-post-subtitle/, 'Blog SPA fallback must render subtitles when present.');
