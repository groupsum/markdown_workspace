...
assert.match(app, /<Route\s+path="\/"\s+element=\{<HomeView\s+isDark=\{isDark\}\s*\/>\}\s*\/>/, 'SPA home route must render HomeView at /.');
assert.match(app, /<Hero\s*\/>\s*<Features\s*\/>\s*<DemoSection\s+isDark=\{isDark\}\s*\/>/s, 'SPA home page must render hero, features, and inline demo in order.');
assert.match(app, /<section\s+id="privacy"\s+className="privacy-section">/, 'SPA home page must render the privacy section.');
assert.match(app, /Your Data Stays on\s*<span className="privacy-heading-accent">Your Device<\/span>/s, 'SPA home privacy heading must keep the device-local promise.');
assert.match(app, /Privacy Standard: Verified Local/, 'SPA home privacy badge must be visible.');
