...
async function captureThemeViewport(browser, themeId, viewport) {
  ...
  await screenshot(page, themeId, viewport.id, 'workspace-panel-open-single-pane');

  const explorerToggleButton = await maybeVisibleLocator(page, 'button[title="Toggle Explorer"]');
  if (explorerToggleButton) {
    await explorerToggleButton.click({ force: true });
    await page.waitForTimeout(200);
    await screenshot(page, themeId, viewport.id, 'workspace-panel-collapsed-single-pane');
    await explorerToggleButton.click({ force: true });
    await page.waitForTimeout(200);
    await screenshot(page, themeId, viewport.id, 'workspace-panel-restored-single-pane');
  }
  ...
  if (splitButton && await splitButton.isEnabled().catch(() => false)) {
    await splitButton.click({ force: true });
    await page.waitForTimeout(200);
    await screenshot(page, themeId, viewport.id, 'workspace-panel-open-split-stage');
    if (explorerToggleButton) {
      await explorerToggleButton.click({ force: true });
      await page.waitForTimeout(200);
      await screenshot(page, themeId, viewport.id, 'workspace-panel-collapsed-split-stage');
      await explorerToggleButton.click({ force: true });
      await page.waitForTimeout(200);
      await screenshot(page, themeId, viewport.id, 'workspace-panel-restored-split-stage');
    }
  }
  ...
  if (previewButton && await previewButton.isEnabled().catch(() => false)) {
    ...
    await page.waitForTimeout(200);
    await screenshot(page, themeId, viewport.id, 'workspace-panel-open-preview-pane');
  }
